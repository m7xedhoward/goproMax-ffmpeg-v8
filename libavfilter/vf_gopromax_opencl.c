...
static int gopromax_opencl_stack(FFFrameSync *fs)
{
    ...
    for (plane = 0; plane < ctx->nb_planes; plane++) {
        ...
        av_log(avctx, AV_LOG_VERBOSE,"In gopromax_opencl_stack for plane:%d %zux%zu frame size %dx%d\n",plane,global_work[0],global_work[1],outlink->w, outlink->h);
...
const FFFilter ff_vf_gopromax_opencl = {
    .p.name            = "gopromax_opencl",
    .p.description     = NULL_IF_CONFIG_SMALL("GoProMax .360 to equirectangular projection"),
    .p.priv_class      = &gopromax_opencl_class,
